...
export const featureWikiContent: Record<string, FeatureWikiEntry> = {
  ...
  "sync-options": {
    overview:
      "Git Sync backs up your vault to a private Git repository (GitHub, GitLab, or any compatible host) and keeps it in sync across devices. It supports both HTTPS (with a Personal Access Token) and SSH (with an SSH key) for authentication, and handles commits, pushes, and pulls automatically — no external git tools required.",
    usage: [
      "Enable Git Sync in the Features list to activate it.",
      "Create a new <strong>private</strong> repository on your Git host — do <strong>not</strong> initialize it with a README or any files (Cascade needs an empty repo).",
      "Open the Git Sync settings page in Cascade and paste the repository URL.",
      "",
      "<strong>── Option A: HTTPS with a Personal Access Token (simpler) ──</strong>",
      "Copy the <strong>HTTPS</strong> URL of your repository (e.g. <code>https://github.com/you/vault.git</code> or <code>https://gitlab.com/you/vault.git</code>).",
      "<strong>GitHub PAT:</strong> Go to github.com → click your avatar → Settings → Developer settings → Personal access tokens → Tokens (classic). Click 'Generate new token', give it a name (e.g. 'Cascade Sync'), select the <strong>repo</strong> scope, and click 'Generate token'. Copy the token immediately (starts with <code>ghp_</code>) — you won't be able to see it again.",
      "<strong>GitLab PAT:</strong> Go to gitlab.com → click your avatar → Edit profile → Access Tokens (in the left sidebar). Click 'Add new token', give it a name, set an expiry date, select the <strong>write_repository</strong> scope, and click 'Create personal access token'. Copy the token (starts with <code>glpat-</code>).",
      "Paste your token into the 'Personal Access Token' field in Cascade. It is stored securely in your operating system's credential store — not in plain text.",
      "",
      "<strong>── Option B: SSH with an SSH key (no token expiry) ──</strong>",
      "Copy the <strong>SSH</strong> URL of your repository (e.g. <code>git@github.com:you/vault.git</code> or <code>git@gitlab.com:you/vault.git</code>). When Cascade detects an SSH URL, it automatically switches to SSH authentication.",
      "<strong>If you already have an SSH key:</strong> Cascade will automatically find keys in the default location (<code>~/.ssh/</code>). It checks for <code>id_ed25519</code>, <code>id_rsa</code>, and <code>id_ecdsa</code> in that order. If your key is elsewhere, enable 'Custom key path' in the settings and enter the full path.",
      "<strong>If you need to create a new SSH key:</strong> Open a terminal and run: <code>ssh-keygen -t ed25519 -C \"your-email@example.com\"</code>. Press Enter to accept the default location (<code>~/.ssh/id_ed25519</code>). You can set a passphrase or leave it empty. This creates two files: a private key (<code>id_ed25519</code>) and a public key (<code>id_ed25519.pub</code>).",
      "<strong>Add your public key to GitHub:</strong> Copy the contents of <code>~/.ssh/id_ed25519.pub</code>. Go to github.com → click your avatar → Settings → SSH and GPG keys → New SSH key. Paste the key and save.",
      "<strong>Add your public key to GitLab:</strong> Copy the contents of <code>~/.ssh/id_ed25519.pub</code>. Go to gitlab.com → click your avatar → Edit profile → SSH Keys (in the left sidebar). Paste the key, set an expiry date (optional), and click 'Add key'.",
      "<strong>Test your connection</strong> (optional): Run <code>ssh -T git@github.com</code> or <code>ssh -T git@gitlab.com</code> in a terminal. You should see a success message with your username.",
      "",
    ...
    tips: [
      "Use a private repository to keep your notes secure.",
      "Any Git host that supports HTTPS or SSH will work — not just GitHub and GitLab.",
      "HTTPS tokens (PATs) may expire — if sync stops working, generate a new token. SSH keys don't expire.",
      "If your SSH key is not in the default <code>~/.ssh/</code> directory, toggle on 'Custom key path' in Sync settings and enter the full path to your private key (e.g. <code>~/keys/my_key</code>).",
